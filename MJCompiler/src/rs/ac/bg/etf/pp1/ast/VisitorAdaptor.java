// generated with ast extension for cup
// version 0.8
// 5/1/2018 19:6:9


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(Extending Extending) { }
    public void visit(DeclarationList DeclarationList) { }
    public void visit(Parameter Parameter) { }
    public void visit(Mulop Mulop) { }
    public void visit(ConstantList ConstantList) { }
    public void visit(Constant Constant) { }
    public void visit(Relop Relop) { }
    public void visit(CondTermList CondTermList) { }
    public void visit(VarName VarName) { }
    public void visit(PostOperator PostOperator) { }
    public void visit(StatementList StatementList) { }
    public void visit(Addop Addop) { }
    public void visit(Factor Factor) { }
    public void visit(CondFactList CondFactList) { }
    public void visit(Designator Designator) { }
    public void visit(Term Term) { }
    public void visit(FormParsList FormParsList) { }
    public void visit(Condition Condition) { }
    public void visit(IdentList IdentList) { }
    public void visit(TypeDef TypeDef) { }
    public void visit(Methods Methods) { }
    public void visit(Brackets Brackets) { }
    public void visit(ExprList ExprList) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(Expr Expr) { }
    public void visit(OptionParameters OptionParameters) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(Decl Decl) { }
    public void visit(Statement Statement) { }
    public void visit(VarDecl VarDecl) { }
    public void visit(CondFact CondFact) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(FormParsDecl FormParsDecl) { }
    public void visit(PrintAdditional PrintAdditional) { }
    public void visit(Mod Mod) { visit(); }
    public void visit(Div Div) { visit(); }
    public void visit(Mul Mul) { visit(); }
    public void visit(Sub Sub) { visit(); }
    public void visit(Add Add) { visit(); }
    public void visit(Lte Lte) { visit(); }
    public void visit(Lt Lt) { visit(); }
    public void visit(Gte Gte) { visit(); }
    public void visit(Gt Gt) { visit(); }
    public void visit(Diff Diff) { visit(); }
    public void visit(Equal Equal) { visit(); }
    public void visit(ElemDesignator ElemDesignator) { visit(); }
    public void visit(ArrayElem ArrayElem) { visit(); }
    public void visit(ClassAccess ClassAccess) { visit(); }
    public void visit(DesignatorName DesignatorName) { visit(); }
    public void visit(ConstBool ConstBool) { visit(); }
    public void visit(ConstChar ConstChar) { visit(); }
    public void visit(ConstInt ConstInt) { visit(); }
    public void visit(FactorParenExpr FactorParenExpr) { visit(); }
    public void visit(NewArray NewArray) { visit(); }
    public void visit(NewClass NewClass) { visit(); }
    public void visit(Const Const) { visit(); }
    public void visit(FactorFuncCall FactorFuncCall) { visit(); }
    public void visit(FactorVar FactorVar) { visit(); }
    public void visit(TermMulop TermMulop) { visit(); }
    public void visit(TermFactor TermFactor) { visit(); }
    public void visit(ExprAddop ExprAddop) { visit(); }
    public void visit(ExprMinusTerm ExprMinusTerm) { visit(); }
    public void visit(ExprTerm ExprTerm) { visit(); }
    public void visit(CondFactWithComparision CondFactWithComparision) { visit(); }
    public void visit(CondFactExpr CondFactExpr) { visit(); }
    public void visit(NoCondFactList NoCondFactList) { visit(); }
    public void visit(ConditionFactList ConditionFactList) { visit(); }
    public void visit(CondTerm CondTerm) { visit(); }
    public void visit(Or Or) { visit(); }
    public void visit(NoCondTermList NoCondTermList) { visit(); }
    public void visit(ConditionTermList ConditionTermList) { visit(); }
    public void visit(CondError CondError) { visit(); }
    public void visit(CondEmptyError CondEmptyError) { visit(); }
    public void visit(ValidCondition ValidCondition) { visit(); }
    public void visit(NoExprList NoExprList) { visit(); }
    public void visit(ExpressionList ExpressionList) { visit(); }
    public void visit(ActPars ActPars) { visit(); }
    public void visit(Dec Dec) { visit(); }
    public void visit(Inc Inc) { visit(); }
    public void visit(NoPars NoPars) { visit(); }
    public void visit(HasPars HasPars) { visit(); }
    public void visit(FuncCallName FuncCallName) { visit(); }
    public void visit(PostOperand PostOperand) { visit(); }
    public void visit(AssignDesignator AssignDesignator) { visit(); }
    public void visit(PostOperation PostOperation) { visit(); }
    public void visit(DesignatorOptMethodCall DesignatorOptMethodCall) { visit(); }
    public void visit(AssignError AssignError) { visit(); }
    public void visit(Assignment Assignment) { visit(); }
    public void visit(NoAdditionalPrint NoAdditionalPrint) { visit(); }
    public void visit(AdditionalPrint AdditionalPrint) { visit(); }
    public void visit(NoStatement NoStatement) { visit(); }
    public void visit(Statements Statements) { visit(); }
    public void visit(LoopBegin LoopBegin) { visit(); }
    public void visit(IfCondition IfCondition) { visit(); }
    public void visit(Else Else) { visit(); }
    public void visit(While While) { visit(); }
    public void visit(MultipleStatements MultipleStatements) { visit(); }
    public void visit(PrintExpr PrintExpr) { visit(); }
    public void visit(Read Read) { visit(); }
    public void visit(ReturnExpr ReturnExpr) { visit(); }
    public void visit(Return Return) { visit(); }
    public void visit(Continue Continue) { visit(); }
    public void visit(Break Break) { visit(); }
    public void visit(DoWhile DoWhile) { visit(); }
    public void visit(IfElse IfElse) { visit(); }
    public void visit(If If) { visit(); }
    public void visit(OperationStatement OperationStatement) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(ParameterError ParameterError) { visit(); }
    public void visit(FormalParameter FormalParameter) { visit(); }
    public void visit(NoFormParsList NoFormParsList) { visit(); }
    public void visit(FormalParameterList FormalParameterList) { visit(); }
    public void visit(NotArray NotArray) { visit(); }
    public void visit(ArrayBracket ArrayBracket) { visit(); }
    public void visit(FormPars FormPars) { visit(); }
    public void visit(NoFormPars NoFormPars) { visit(); }
    public void visit(FormParameters FormParameters) { visit(); }
    public void visit(TypeVoid TypeVoid) { visit(); }
    public void visit(SomeType SomeType) { visit(); }
    public void visit(NoVarDecl NoVarDecl) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(MethodTypeName MethodTypeName) { visit(); }
    public void visit(MethodVarEnd MethodVarEnd) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(NoMethodList NoMethodList) { visit(); }
    public void visit(MethodList MethodList) { visit(); }
    public void visit(NoClassMethods NoClassMethods) { visit(); }
    public void visit(ClassHasMethods ClassHasMethods) { visit(); }
    public void visit(ExtendError ExtendError) { visit(); }
    public void visit(NoExtend NoExtend) { visit(); }
    public void visit(Extends Extends) { visit(); }
    public void visit(ClassName ClassName) { visit(); }
    public void visit(ClassStart ClassStart) { visit(); }
    public void visit(ClassVars ClassVars) { visit(); }
    public void visit(ClassDecl ClassDecl) { visit(); }
    public void visit(VarOne VarOne) { visit(); }
    public void visit(VarNameList VarNameList) { visit(); }
    public void visit(NoName NoName) { visit(); }
    public void visit(InvalidName InvalidName) { visit(); }
    public void visit(VarNameDecl VarNameDecl) { visit(); }
    public void visit(VarDeclError VarDeclError) { visit(); }
    public void visit(VarDeclGood VarDeclGood) { visit(); }
    public void visit(ValueAssign ValueAssign) { visit(); }
    public void visit(NoConstList NoConstList) { visit(); }
    public void visit(ConstList ConstList) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(DeclDerived3 DeclDerived3) { visit(); }
    public void visit(DeclDerived2 DeclDerived2) { visit(); }
    public void visit(DeclDerived1 DeclDerived1) { visit(); }
    public void visit(DeclarationListEmpty DeclarationListEmpty) { visit(); }
    public void visit(DeclList DeclList) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
