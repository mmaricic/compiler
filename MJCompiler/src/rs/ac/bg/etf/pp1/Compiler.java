...
public class Compiler {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 2) {
			...
			System.err.println("Nedovoljno argumenata! Koriscenje: <source-file> <obj-file> ");
			return;
		}
		...
		if (!sourceCode.exists()) {
			System.err.println("Izvorni fajl [" + sourceCode.getAbsolutePath() + "] nije pronadjen!");
		...
		try{
			...
			if (!p.errorDetected && semanticCheck.passed()) {
				...
				log.info("Generise se bajtkod: " + objFile.getAbsolutePath());
				if (objFile.exists())
					objFile.delete();
